...
void setup() {
  ...
  Firebase.begin(SECRET_FIREBASE_HOST, SECRET_FIREBASE_AUTH, SECRET_SSID, SECRET_OPTIONAL_PASS);
  // reinitialize firebase connection is wifi lost
...
void loop() {
  ...
  String jsonData = "{\"date\":{ \".sv\":\"timestamp\"}, \"moisture\":" + String(moisture_percent) + ", \"humidity\":" + String(humidity) + ", \"temperature\":" + String(temperature) + ", \"pump_trigger\":" + String(pump_trigger) + ", \"pump_status\":" + String(pump_status) + "}";
  
  if (Firebase.setJSON(fbdo, "/firebase-iot/live-data", jsonData)){
  ...
  //Failed, then print out the error detail
    Serial.println("In firebase live data upload");
    Serial.println(fbdo.errorReason());
}


if (Firebase.push;/JSON(fbdo, "/firebase-iot/past-data", jsonData)) {
  ...
  Serial.println("In firebase past data upload");
  //Failed, then print out the error detail
  Serial.println(fbdo.errorReason());
}
